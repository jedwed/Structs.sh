...
export default class AVLAnimationProducer extends AnimationProducer {
  ...
  // the problem with each AVLAnimationProducer having its own draw canvas created
  ...
  public updateNodesRecursive(node: Node, animation: Runner[]): void {
    ...
    AVLAnimationProducer.updateNode(node, node.x, node.y, animation);
  ...
  public updateLinesRecursive(node: Node, animation: Runner[]): void {
    ...
    AVLAnimationProducer.updateNodeLines(node, animation);
  ...
  public resetAVL(root: Node): void {
    const animation: Runner[] = [];
  ...
  public resetLinesRecursive(node: Node, animation: Runner[]): void {
    ...
    AVLAnimationProducer.unhighlightLine(node.leftLineTarget, node.leftArrowTarget, animation);
    AVLAnimationProducer.unhighlightLine(node.rightLineTarget, node.rightArrowTarget, animation);
  ...
  public resetNodesRecursive(node: Node, animation: Runner[]): void {
    ...
    AVLAnimationProducer.unhighlightNode(node, animation);
